        ...
        public string Decode(byte[] bytes)
        {
            ...
            for(int i=0; i<bytes.Length;)
            {
                ...
                if (chr >= 0x80)
                {
                    ...
                    {
                        // Character cannot be decoded with charset - this shouldn't happen with proper files
                        // Use the high byte as-is rather than trying UTF-8 or ISO-8859-1 detection
                        chr = Convert.ToChar(bytes[i - 1]);
                    }
                }
